...
1. Create the necessary files. Will need:
    ...
    * Manifest file
    * Possibly an index.html that will pop up when someone click the extension icon

2. Scripts page scripts (https://developer.chrome.com/docs/extensions/mv3/content_scripts/)
    * Ideal outcome: Someone can send an email and the email will be added to Mautic in the user card history. 
        Tracking opens and adding to card history would be cool too
    * Create the content scripts (might need background scripts for some operations, not sure yet)
        ...
        - Can be injected statically (in the manifest.json file, shown above) or programatically. 
            For programatic insertion, extension needs host permissions for the page that it's injecting script into. 
            For reference: https://developer.chrome.com/docs/extensions/mv3/content_scripts/
    * Algorithm for scripts:
        ...
        - On email composition card, create a box that a user can check. This is to tell Mautic that an email 
            has been sent to a contact
